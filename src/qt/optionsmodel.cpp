...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeInstaCashAmount"))
        settings.setValue("nAnonymizeInstaCashAmount", 1000);

    nAnonymizeInstaCashAmount = settings.value("nAnonymizeInstaCashAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeInstaCashAmount"))
        SoftSetArg("-anonymizeinstacashamount", settings.value("nAnonymizeInstaCashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in instacash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeInstaCashAmount:
            return QVariant(nAnonymizeInstaCashAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeInstaCashAmount:
            nAnonymizeInstaCashAmount = value.toInt();
            settings.setValue("nAnonymizeInstaCashAmount", nAnonymizeInstaCashAmount);
            emit anonymizeInstaCashAmountChanged(nAnonymizeInstaCashAmount);
            break;
        case CoinControlFeatures:
