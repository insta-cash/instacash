...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a InstaCash address (e.g. %1)").arg("A7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no InstaCash: URI
...
{
    // Convert instacash:// to instacash:
    //
    //    Cannot handle this later, because instacash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open instacash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "InstaCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for InstaCash.lnk
...
{
    return GetAutostartDir() / "instacash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a instacash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=InstaCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the instacash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add instacash app to startup item list
